...
export const getEventTypeLabel = (type: string): string => {
  const labels: Record<string, string> = {
    ...
    peasant_revolt: 'Крестьянское восстание',
    land_enclosure: 'Огораживание',
    crop_failure: 'Неурожай',
    factory_conditions: 'Условия труда',
    luddite_rebellion: 'Восстание луддитов',
    urbanization: 'Урбанизация',
    trust_busting: 'Антимонопольное регулирование',
    stock_crash: 'Биржевой крах',
    trade_wars: 'Торговые войны',
    automation_shock: 'Автоматизация',
    green_movement: 'Экологическое движение',
    financial_crisis: 'Финансовый кризис',
    nationalization: 'Национализация',
    worker_cooperative: 'Рабочее самоуправление',
  };
...
export const getEventTypeColor = (type: string): string => {
  const colors: Record<string, string> = {
    economic_crisis: 'bg-red-500/10 text-red-400',
    technological_breakthrough: 'bg-emerald-500/10 text-emerald-400',
    social_movement: 'bg-blue-500/10 text-blue-400',
    political_change: 'bg-purple-500/10 text-purple-400',
    war: 'bg-red-500/10 text-red-400',
    market_change: 'bg-amber-500/10 text-amber-400',
    labor_conflict: 'bg-orange-500/10 text-orange-400',
  };
  return colors[type] || 'bg-slate-800/40 text-slate-400';
};
...
export const getCrisisPhaseColor = (phase: string): string => {
  ...
  return colors[phase as keyof typeof colors] || 'text-slate-500';
};
