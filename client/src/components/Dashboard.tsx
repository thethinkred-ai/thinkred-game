...
import { computePercentChange, formatTrend, TrendInfo } from '../utils/trends';
...
function TrendBadge({ trend }: { trend: TrendInfo }) {
  if (trend.positive === null) {
    return <div className="stat-change text-slate-500"><span>—</span><span>{trend.label}</span></div>;
  }
  return (
    <div className={`stat-change ${trend.positive ? 'stat-change-positive' : 'stat-change-negative'}`}>
...
export const Dashboard: React.FC = () => {
  ...
  const [previousSummary, setPreviousSummary] = useState<SnapshotSummary | null>(null);

  useEffect(() => {
    apiService.getEconomicHistory()
      .then((history) => {
        if (history.length >= 2) {
          const prev = history[history.length - 2]?.summary;
          setPreviousSummary(prev ?? null);
        } else if (history.length === 1) {
          setPreviousSummary(history[0]?.summary ?? null);
        } else {
          setPreviousSummary(null);
        }
      })
      .catch(() => setPreviousSummary(null));
  }, [gameState.enterprises, gameState.economicIndicators]);
  ...
  const trends = useMemo(() => {
    ...
    const avgSurplusValue = gameState.enterprises.length > 0
      ...
      : 0;
    const enterpriseCount = gameState.enterprises.length;

    if (!previousSummary) {
      const empty: TrendInfo = { percent: null, label: 'Недостаточно данных', positive: null };
      return { profit: empty, workers: empty, surplus: empty, enterprises: empty };
    }

    return {
      profit: formatTrend(computePercentChange(totalProfit, previousSummary.totalProfit)),
      workers: formatTrend(computePercentChange(totalWorkers, previousSummary.totalWorkers)),
      surplus: formatTrend(computePercentChange(avgSurplusValue, previousSummary.avgSurplusValue)),
      enterprises: formatTrend(
        computePercentChange(enterpriseCount, previousSummary.enterpriseCount),
        'с последнего снимка'
      ),
    };
  }, [gameState.enterprises, previousSummary]);
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              <TrendBadge trend={trends.profit} />
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              <TrendBadge trend={trends.workers} />
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              <TrendBadge trend={trends.surplus} />
            </div>
        ...
        <div className="stat-card">
          <div className="flex items-start justify-between">
            <div className="space-y-1">
              ...
              <TrendBadge trend={trends.enterprises} />
            </div>
