...
import { migrateGameEventsColumns, migrateEnterprisePeriod, migrateAchievements, migrateUserSecurityColumns } from './migrations';
...
function runMigrations(db: SqlJsDatabase) {
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS stepik_tokens (
      user_id INTEGER PRIMARY KEY REFERENCES users(id),
      stepik_id INTEGER NOT NULL,
    ...
    CREATE TABLE IF NOT EXISTS game_progress_log (
      id INTEGER PRIMARY KEY AUTOINCREMENT,
      user_id INTEGER NOT NULL REFERENCES users(id),
      event_type TEXT NOT NULL,
  ...
  db.run(`
    CREATE TABLE IF NOT EXISTS decision_cooldowns (
      user_id INTEGER PRIMARY KEY REFERENCES users(id),
      last_decision_at TEXT NOT NULL DEFAULT (datetime('now', '-1 day'))
    );
  `);
}
